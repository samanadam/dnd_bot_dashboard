...
| A session is deleted or renamed by the wrong person, by mistake, or to reach files outside it | Both calls are `dmOnly` and audited. The delete body must repeat the session id in the path, and the page asks the DM to type `delete`. The bot refuses while the session is recording or the transcriber holds it, removes files before the database row so a failure can be retried, only ever deletes folders directly under its own data directories (never following a symlink), and rate limits the call like recording controls. The transcriber's own archive is not reachable and is left alone | `lib/bot/allowlist.ts`, `dnd_bot/api/routes_sessions.py`, `dnd_bot/session_admin.py` |
