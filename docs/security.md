...
| Unlicensed item text in the public repo | The Open5e item endpoints ignore the document filter and mix in other books (Vault of Magic among them). The item importer keeps an entry only if its own `document.key` is `srd-2014` or `srd-2024` and drops everything else; a test also checks the bundled files for any other book | `scripts/import-srd-items.mts`, `lib/dm/openItems.ts`, `tests/items.test.ts` |
